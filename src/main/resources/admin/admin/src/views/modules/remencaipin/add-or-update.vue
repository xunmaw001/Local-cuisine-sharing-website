...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirongxiangqing = this.ruleForm.neirongxiangqing.replace(reg,'../../../springbooth10zf/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `remencaipin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.remencaipinCrossAddOrUpdateFlag = false;
						 this.parent.search();
